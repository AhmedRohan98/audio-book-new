...
if ($get_method['method_name'] == "home") {
	...
	mail($to, $recipient_name, $subject, $message);
	...
	if ($_FILES['user_image']['name'] != "") {
		...
		if ($ext != 'png') {
			...
			move_uploaded_file($_FILES['user_image']['tmp_name'], $thumbpath);
		}
	...
	// echo $row['user_type'].'__'.$user_image;
	...
	if (!filter_var($get_method['email'], FILTER_VALIDATE_EMAIL)) {
		...
		$sql_au = "SELECT * FROM tbl_users WHERE `id` = '" . $get_method['user_id'] . "'";
		$result_au = mysqli_query($mysqli, $sql_au);
		$row_au = mysqli_fetch_assoc($result_au);

		if ($row_au['user_type'] == "Normal" || $row_au['user_type'] == "normal") {
			$user_image_2 =  $file_path . 'images/' . $user_image;
		} else {
			$user_image_2 = $user_image;
		}


		$set['AUDIO_BOOK'][] = array('msg' => $app_lang['update_success'], 'name' => $row_au['name'], 'email' => $row_au['email'], 'phone' => $row_au['phone'], 'user_image' => $user_image_2, 'success' => '1');
	}
	...
	echo $val = str_replace('\\/', '/', json_encode($set, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
	die();
} else if ($get_method['method_name'] == "forgot_pass_2") {
	$host = $_SERVER['HTTP_HOST'];
	preg_match("/[^\.\/]+\.[^\.\/]+$/", $host, $matches);
	$domain_name = $matches[0];


	$qry = "SELECT * FROM tbl_users WHERE email = '" . $get_method['user_email'] . "' AND `user_type`='Normal' AND `id` <> 0";
	$result = mysqli_query($mysqli, $qry);
	$row = mysqli_fetch_assoc($result);
	...
	if ($row['email'] != "") {
		...
		$set['EBOOK_APP'][] = array('msg' => "Password has been sent on your mail!", 'success' => '1');
	} else {

		$set['EBOOK_APP'][] = array('msg' => "Email not found in our database!", 'success' => '0');
	}
